...
from s3_backend import get_tracking_log_objects, get_sql_data_objects
from load_course_sql import load_local_sql_files_to_bigquery
...
def get_course_ids_no_check(args):
    if type(args)==str or type(args)==unicode:		# special case: a single course, already specified
        return [ args ]
    if type(args)==list:
        return args
    ...
    if sqlall or 'sql2bq' in steps:
        ...
        try:
            if param.use_local_files:
                load_local_sql_files_to_bigquery(
                    course_id=course_id,
                    verbose=args.verbose,
                    base_dir=param.the_basedir,
                    date_dir=param.the_datedir,
                )
            else:
                load_course_sql.load_sql_for_course(
                    course_id,
...
def get_mysql_data_from_simple_storage_service(param, args):
    """
    Gets and downloads the SQL data generated by edx-analytics-exporter.

    Args:
        param: Commad Parameters object.
        args: Command arguments.
    Raises:
        Exception: If --start-date value was not provided.
        Any Exception raised by the process.
    """
    if not getattr(param, 'start_date', None):
        raise Exception('--start-date must be specified in this format: YYYY-MM-DD.')

    get_sql_data_objects(
        bucket_name=getattr(edx2bigquery_config, 'AWS_BUCKET_NAME', None),
        start_date=param.start_date,
    )
...
def analyze_videos(param, courses, args):
    ...
    for course_id in get_course_ids(courses):
        try:
            make_video_analysis.analyze_videos(
                course_id,
                use_local_files=param.use_local_files,
...
def axis2bq(param, courses, args, stop_on_error=True):
    ...
    for course_id in get_course_ids(courses):
        ...
        make_course_axis.process_course(
            course_id,
            param.the_basedir,
            param.the_datedir,
...
def person_course(param, courses, args, just_do_nightly=False, force_recompute=False):
    ...
    for course_id in get_course_ids(courses):
        try:
            make_person_course.make_person_course(
                course_id,
                gsbucket=edx2bigquery_config.GS_BUCKET,
                basedir=param.the_basedir,
                datedir=param.the_datedir,
...
logsfromS3                  : Search and download all the object matched by --start-date argument from Amazon S3.
                              ...
                              These settings must be speceified in the edx2bigquery_config.py file:
                              AWS_ACCESS_KEY_ID
...
sqlfromS3                   : Search and download all the MySQL objects from Amazon S3.
                              These settings must be speceified in the edx2bigquery_config.py file:
                              AWS_ACCESS_KEY_ID
                              AWS_SECRET_ACCESS_KEY
                              AWS_BUCKET_NAME
                              SQL_DATA_BUCKET_PATH: Path where the MySQL data are stored inside of the bucket.
                              SQL_FILE_NAME_PREFIX: Prefix of the zip file name without the date.
                              SQL_LOCAL_FOLDER: Name of the local folder to store the .zip SQL data.
                              SQL_SOURCE_DATA_LOCAL_FOLDER: Name of the local folder to uncompress the SQL .zip data.
...
"""
    ...
    elif args.command == 'sqlfromS3':
        get_mysql_data_from_simple_storage_service(param, args)
